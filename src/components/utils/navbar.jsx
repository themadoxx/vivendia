...
const Navbar = () => {
  ...
  // Gestion du clic extérieur pour fermer le menu mobile
  useEffect(() => {
  ...
  return (
    <nav className="bg-white shadow-md lg:relative sticky top-0 z-20 w-full">
      <div className="flex justify-between items-center px-5 py-3 h-[70px] max-w-[1200px] mx-auto relative">
        
        {/* Logo */}
        <div className="flex items-center">
          <a href="/">
            <img src={`${process.env.PUBLIC_URL}/vivendia-logo.svg`} alt="Vivendia" className="h-10" />
          </a>
        </div>
          ...
          <ul className="flex flex-row items-center space-x-8">
            {[
              ...
              <li key={index} className="relative">
                <a href={item.href} className="text-gray-800 text-sm font-medium py-2 px-1 hover:text-red-600 transition-colors duration-300">
                  {item.title}
                </a>
              </li>
            ))}
        ...
        <div 
          ref={menuRef}
          className={`fixed lg:hidden top-0 ${isOpen ? 'right-0' : '-right-full'} h-screen w-[250px] bg-white shadow-lg transition-all duration-300 ease-in-out z-50 pt-24`}
        >
          <ul className="flex flex-col items-start p-0 px-5 space-y-6">
            {[
              ...
              <li key={index} className="w-full">
                <a href={item.href} className="text-gray-800 text-sm font-medium block py-2 px-1.5 hover:text-red-600 transition-colors duration-300">
                  {item.title}
                </a>
              </li>
            ))}
